...
struct private_data_s
{
  ...
  bool maskdir_bind_failed;     /* Fast-path: skip bind attempts if they fail */
  bool maskdir_warned;          /* Track if we've warned about fallback for this container */
};
...
static void
warn_fallback_once (struct private_data_s *private_data, const char *reason)
{
...
mount_masked_dir (libcrun_container_t *container, int pathfd, const char *rel_path, libcrun_error_t *err)
{
  ...
  /* Fast-path: if bind mounts failed before, skip directly to tmpfs */
  ...
  if (ret < 0)
    {
      /* Shared directory creation failed, mark bind as failed and fall back */
      private_data->maskdir_bind_failed = true;
      if (tmp_err != NULL)
        {
          warn_fallback_once (private_data, tmp_err->msg);
...
fallback_to_tmpfs:
  return do_mount (container, "tmpfs", pathfd, rel_path, "tmpfs", MS_RDONLY, "size=0k", LABEL_MOUNT, err);
}
